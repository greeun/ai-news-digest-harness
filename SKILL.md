...
description: |
  ...
  웹에서 최신 AI 뉴스를 수집하고, 한국어로 요약하며, 정갈한 디자인의 self-contained HTML 페이지로 제공.
  트리거 — EN: "AI news digest", "AI news summary", "daily AI news", "AI newsletter",
  "trending AI news", "AI news page", "generate AI digest".
  KO: "AI 뉴스", "AI 소식", "인공지능 뉴스", "AI 뉴스 요약", "AI 뉴스 정리",
  "오늘의 AI 뉴스", "AI 트렌드", "AI 뉴스 다이제스트", "AI 뉴스 페이지",
  "AI 소식 정리", "최신 AI 소식", "AI 뉴스 모아줘".
---

# AI News Digest Harness

최신 AI 뉴스를 웹에서 수집하고, 읽기 쉬운 한국어로 요약하여, 모던하고 정갈한 디자인의 단일 HTML 페이지로 제공하는 하네스 기반 스킬.

Planner가 실제 뉴스를 리서치하고, Generator가 HTML 페이지를 제작하며, Evaluator가 콘텐츠 완성도와 디자인 품질을 독립적으로 검증한다.

## 이 하네스가 방지하는 두 가지 실패

1. **자기평가 편향 (Self-evaluation bias).** 뉴스를 수집하고 HTML을 만드는 에이전트가 스스로 "잘 만들었다"고 판단하면, 누락된 뉴스, 어색한 한국어, 조잡한 디자인을 놓친다. 별도의 Evaluator가 skeptical하게 검증해야 품질이 보장된다.
...
1. **구조적 역할 분리**: Planner(뉴스 리서치), Generator(HTML 제작), Evaluator(품질 검증)는 반드시 별도 서브에이전트로 실행. 하나의 세션에서 역할극 금지.
...
사용자가 AI 뉴스 다이제스트를 요청하면:

### Step 1: 작업 디렉토리 설정
```
working_dir = ./ai-news-digest-output/
filename = ai-news-digest-{YYYYMMDD-HHmmss}.html   # 예: ai-news-digest-20260416-143025.html
```
작업 디렉토리를 생성하고 모든 파일을 여기에 저장. HTML 파일명에 생성 시점의 datetime을 포함.
...
- Planner는 WebSearch/WebFetch로 실제 최신 AI 뉴스를 검색하고 수집
- `spec.md` 작성 완료까지 대기
- **사용자에게 spec.md 내용을 보여주고 확인 받기** (필수 게이트)

### Step 3: Generator 서브에이전트 디스패치

```
Agent({
  description: "AI News HTML Generator",
...
```
Agent({
  description: "AI News Evaluator",
  prompt: [references/evaluator-prompt.md 내용] + "\n\n작업 디렉토리: {working_dir}",
  mode: "auto"
})
```

- Evaluator는 `spec.md`, `sprint_contract.md`, `generator_report.md`, `ai-news-digest.html`을 읽고 검증
...
if critique.md verdict == "PASS":
    # 완료! 사용자에게 결과 보고
    # ai-news-digest-{datetime}.html을 브라우저에서 열기 제안

if iteration_cap(3) 도달 && still FAIL:
    # 사용자에게 현재 상태 보고 및 수동 개입 요청
```

**비선형 품질 향상 주의**: 원문 경험에 따르면, 마지막 이터레이션이 항상 최선은 아니다. 중간 이터레이션이 더 나은 경우가 있으므로, 각 이터레이션의 HTML을 `ai-news-digest-{datetime}-v{N}.html`로 보존하고, Evaluator 점수가 가장 높은 버전을 최종 산출물로 선택한다.

### Step 6: 결과 전달

- 완성된 `ai-news-digest.html` 경로를 사용자에게 안내
- `open {working_dir}/ai-news-digest.html`로 브라우저에서 확인 제안
...
**통과 기준**: 모든 항목 >= 4, 2x 가중 항목(요약 품질, 디자인 품질) 둘 다 >= 4.
...
- **Evaluator가 반드시 필요한 조건**: 디자인 품질이 중요하거나, 뉴스 아이템이 10개 이상이거나, 한국어 요약의 정확성이 중요할 때

## 파일 핸드오프 계약 (File Handoff Contract)

```
Files (single source of truth):
  spec.md                                  Planner → Generator, Evaluator  (뉴스 목록 + 콘텐츠 스펙)
  sprint_contract.md                       Generator ↔ Evaluator           (무엇을 만들고 어떻게 검증할지)
  generator_report.md                      Generator → Evaluator            (작업 결과 보고)
  critique.md                              Evaluator → Generator            (루브릭 점수 + 블로킹 이슈)
  handoff.md                               outgoing → fresh session         (컨텍스트 리셋 시)
  ai-news-digest-{YYYYMMDD-HHmmss}.html   Generator output                 (최종 산출물, datetime 포함)
  ai-news-digest-{datetime}-v{N}.html      이터레이션별 버전 보존           (비선형 품질 대응)
```
